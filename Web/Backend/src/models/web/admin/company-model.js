...
function getCompanyList(uid, data) {
    return new Promise((resolve, reject) => {
        ...
        let query = `select c.*, c.companyID as ID, 
                    (select ifnull(count(m.userID) from, 0) ` + table.USERS + ` m left join ` + table.USER_RELATIONSHIP + ` usr on usr.userID = m.userID and usr.type = 'building' left join buildings on usr.relationID = buildings.buildingID where m.usertype = 'manager' and m.permission = 'active' and buildings.companyID = c.companyID group by m.userID) as manager_count,
